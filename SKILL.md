...
- `examples/redteam-*.html`: red-team fixtures — deliverables that actively try to sneak past the gates; the eval suite proves each is rejected. `examples/redteam-original-failure.html` is the exact failure class v1.0 exists to prevent (and the off-palette anti-pattern `references/elevate.md` points to); `examples/redteam-premium-clean-template.html` and `examples/redteam-premium-gamed.html` attack the Premium tier bar; `examples/redteam-conformance-evasion.html` attacks the structure check.
...
- `examples/dogfood-premium/`: a real end-to-end v1.1 Premium-tier run — Premium brief with a structure spec, `deep`-tier concepts, a clean-template draft caught by the tier gate alone (brand and conformance both passed it), and the rebuilt winner passing every gate.
...
- `scripts/validate-conformance.py`: evasion-resistant structure check against the brief's declared structure spec — required sections present, ordered, non-stub, and visible; banned markup caught through reformatting; required placeholders visibly present (a hidden placeholder over a guessed value is named as dishonesty); exits non-zero on a violation.
...
- `docs/v1.1-acceptance-report.md`: the three v1.1 guarantees (tier bar, conformance, brand), each mapped to its proving evals.
